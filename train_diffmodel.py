...
def main(args):
    ...
    if args.resume_from_checkpoint:
        sd = torch.load(cfg.train.doubleunet_path, map_location="cpu")
        diffmodel.load_state_dict(sd, strict=True)

        if accelerator.is_main_process:
            logger.info(f"load DoubleUNet from {cfg.train.doubleunet_path}")
    else:
        logger.info("Initializing DoubleUNet weights from scratch")
        pass
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(diffmodel):
                ...
                pixel_values = ((batch["pixel_values"]+1)/2).to(accelerator.device)
                ...
                #可以尝试多种Loss
                diff = pixel_values - controlnet_image
                diff = diff.abs()
                loss = F.l1_loss(mask_image, diff, reduction="sum")
