...
class StableDiffusionXLControlNeXtPipeline(
    ...
    def _get_add_time_ids(
        self, original_size, crops_coords_top_left, aesthetic_score,target_size, dtype, text_encoder_projection_dim=None
    ):
    ...
    def __call__(
        ...
        diff: Optional[PipelineImageInput] = None,
        ...
        aesthetic_score: Optional[int] = 8,
        **kwargs,
    ):
        r"""
        ...
        if diff is not None and self.controlnet is not None:
            diff = self.prepare_image(
                diff,
                width,
                height,
                batch_size,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if h * w <= tile_size * tile_size:
                    ...
                    #     controls['out'] = torch.cat([torch.zeros_like(controls['out']), controls['out']])



                    # # This makes the effect of the controlnext much more stronger
                    # if  self.do_classifier_free_guidance:
                    #     controls['out'] = torch.cat([controls['out']]*2)
                    #     scale = controls['scale']
                    #     scale = torch.tensor([scale],device=controls['out'].device)
                    ...
                    for row in range(grid_rows):
                        ...
                        for col in range(grid_cols):
                            ...
                            img_tile = controlnet_image[:, :, input_start_y * 8:input_end_y * 8, input_start_x * 8:input_end_x * 8]
                            img_list.append(img_tile)
                            diff_tile = diff[:, :, input_start_y * 8:input_end_y * 8,input_start_x * 8:input_end_x * 8]
                            diff_list.append(diff_tile)

                            if len(input_list) == batch_size or col == grid_cols - 1:
                                input_list_t = torch.cat(input_list, dim=0)
                                img_list_t = torch.cat(img_list, dim=0)
                                diff_list_t = torch.cat(diff_list, dim=0)
