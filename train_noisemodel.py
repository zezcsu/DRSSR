...
def main(args):
    ...
    noiseresnet: NoiseResNet = instantiate_from_config(cfg.model.noiseresnet)
    if args.resume_from_checkpoint:
        loadnet = torch.load(cfg.train.noiseresnet_path, map_location="cpu")
        noiseresnet.load_state_dict(loadnet, strict=True)
        if accelerator.is_main_process:
    ...
    # Optimizer creation
    noiseresnet.train().to(accelerator.device)
    noiseresnet.requires_grad_(True)
    params_to_optimize = [{'params': list( noiseresnet.parameters()), 'lr': args.learning_rate}]
    logger.info(
        f"Number of trainable parameters in diffmodel: {sum(p.numel() for p in  noiseresnet.parameters() if p.requires_grad)}")
    ...
    diffmodel, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        noiseresnet, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate( noiseresnet):
                ...
                # timesteps = torch.full((bsz,), 199, device=lr_latents.device)
                # timesteps = timesteps.long()
                # hr_latents_noise = noise_scheduler.add_noise(hr_latents, noise, timesteps)
                # lr_latents_noise = noise_scheduler.add_noise(lr_latents, noise, timesteps)

                sr_latents =  noiseresnet(lr_latents)
                #可以尝试多种Loss

                loss = F.l1_loss(sr_latents, hr_latents , reduction="sum")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        save_models(
                            accelerator.unwrap_model( noiseresnet),
                            save_path,
                        )
