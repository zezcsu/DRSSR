...
def log_validation(noisemodel, noise_scheduler,vae, unet, controlnet, args, accelerator, weight_dtype, step,
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        if args.pretrained_vae_model_name_or_path is not None:
            validation_image = validation_image.to(accelerator.device,dtype=weight_dtype)
        ...
        with torch.cuda.amp.autocast(enabled=True):
            sr_latents = noisemodel(lr_latents)
...
def main(args):
    ...
    noisemodel: NoiseResNet = instantiate_from_config(cfg.model.noiseresnet)
    loadnet = torch.load(cfg.train.noiseresnet_path, map_location="cpu")
    noisemodel.load_state_dict(loadnet, strict=True)
    for p in noisemodel.parameters():
        p.requires_grad = False
    if accelerator.is_main_process:
        logger.info(f"load NoiseResNet from {cfg.train.noiseresnet_path}")
    ...
    noisemodel.eval().to(accelerator.device)
