...
"""
Watchdog caught collective operation timeout: WorkNCCL(SeqNum=2, OpType=ALLREDUCE, NumelIn=1, NumelOut=1, Timeout(ms)=600000) ran for 600026 milliseconds before timing out.
"""
...
if num_gpus > 1:
    ...
    dist.init_process_group(
        ...
        timeout=datetime.timedelta(seconds=12000)  # 200 分钟超时
    )
    print(f"分布式训练已初始化，使用 {num_gpus} 张 GPU")
else:
    print("仅检测到 1 张 GPU，不启用分布式训练")
...
def log_validation(vae, unet, controlnet, srmodel, diffmodel, args, accelerator, weight_dtype, step,
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        # 计算扩大四倍后的尺寸（宽度和高度各乘以2）
        new_width = original_width * 4
        new_height = original_height * 4

        # 使用resize方法扩大图像，可以选择不同的重采样方法
        # 这里使用Image.BILINEAR（双线性插值）或Image.LANCZOS（高质量下采样）
        log_validation_image = validation_image.resize((new_width, new_height), Image.LANCZOS)
        ...
        with torch.no_grad():
            validation_image = srmodel(validation_image)
            diff = diffmodel(lr, validation_image)

        validation_image = validation_image.clamp(0, 1)

        validation_image.to(accelerator.device, dtype=weight_dtype)
        diff = diff.to(accelerator.device, dtype=weight_dtype)
        images = []
...
def prepare_train_dataset(dataset, accelerator, convert_image_to="RGB", center_crop=False, random_flip=True):
    ...
    def preprocess_train(examples):
        if crop_preproc is not None:
            ...
            # 将结果转换为列表（zip返回的是元组）
            hrs = list(hrs)
            # hrs = [ img_preproc(image) for image in hrs]
            lrs = list(lrs)
...
def main(args):
    ...
    if accelerator.is_main_process:
        logger.info(f"load SwinIR from {cfg.train.swinir_path}")
    ...
    for p in diffmodel.parameters():
        p.requires_grad = False
    if accelerator.is_main_process:
        logger.info(f"load DoubleUNet from {cfg.train.doubleunet_path}")
    ...
    #     pass
    if args.pretrained_unet_model_name_or_path:
        logger.info("Loading partial unet weights")
        unet_sd = load_file(args.pretrained_unet_model_name_or_path)  # 加载部分权重

        # 获取当前UNet的完整state_dict
        current_unet_sd = unet.state_dict()

        # 选择性更新权重（仅覆盖存在的键）
        for k in unet_sd.keys():
            if k in current_unet_sd:
                if args.load_weights_increaments:  # 增量加载模式
                    current_unet_sd[k] += unet_sd[k]  # 累加权重
                else:  # 直接覆盖模式
                    current_unet_sd[k] = unet_sd[k]
            else:
                logger.warning(f"Ignored key '{k}' (not found in current UNet)")

        # 加载更新后的权重
        unet.load_state_dict(current_unet_sd, strict=False)  # strict=False允许部分加载
    else:
        logger.info("Initializing unet weights from scratch")
    ...
    swinir.eval().to(accelerator.device)
    diffmodel.eval().to(accelerator.device)
    ...
    logger.info(
        f"Number of trainable parameters in controlnet: {sum(p.numel() for p in controlnet.parameters() if p.requires_grad)}")

    # 允许一部分可以训练，减少control控制难度，特别是在引入condition的时候
    unet.train()
    ...
    logger.info(
        f"Number of trainable parameters in unet: {sum(p.numel() for p in unet.parameters() if p.requires_grad)}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, controlnet):
                ...
                with torch.no_grad():
                    controlnet_image = swinir(controlnet_image)
                    mask_image = diffmodel(lr, controlnet_image)

                controlnet_image = controlnet_image.clamp(0, 1)
                controlnet_image = controlnet_image.to(accelerator.device, dtype=weight_dtype)
                mask_image = mask_image.to(accelerator.device, dtype=weight_dtype)
