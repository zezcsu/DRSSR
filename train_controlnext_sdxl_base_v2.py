...
from pipeline.pipeline_controlnext_base_v2 import StableDiffusionXLControlNeXtPipeline
from models.controlnetv2 import ControlNetModel
...
def log_validation(diffmodel, sr_model ,vae, unet, controlnet, args, accelerator, weight_dtype, step,
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            mask = diffmodel(validation_image)
            validation_image = sr_model(validation_image)
...
def main(args):
    ...
    if accelerator.is_main_process:
        print(f"load HAT from {cfg.train.hat_path}")

    diffmodel: UNetResNet50 = instantiate_from_config(cfg.model.unetresnet50)
    loadnet = torch.load(cfg.train.unetresnet50_path, map_location="cpu")
    ...
    for p in diffmodel.parameters():
        p.requires_grad = False
    if accelerator.is_main_process:
        logger.info(f"load HAT from {cfg.train.unetresnet50_path}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, controlnet):
                ...
                mask = diffmodel(controlnet_image)
                DR_controlnet_image = hat(controlnet_image)
                ...
                loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")+(F.mse_loss(model_pred.float(), target.float(), reduction="none")*mask).mean()
