...
def save_models(model, output_dir):
    ...
    torch.save(model.state_dict(), os.path.join(output_dir, 'srmodel.pth'))
...
def main(args):
    ...
    if args.resume_from_checkpoint:
        ...
        logger.info("Initializing SwinIR weights from scratch")
        pass
    ...
    # Optimizer creation
    swinir.train().to(accelerator.device)
    swinir.requires_grad_(True)
    params_to_optimize = [{'params': list(swinir.parameters()), 'lr': args.learning_rate}]
    logger.info(
        f"Number of trainable parameters in diffmodel: {sum(p.numel() for p in swinir.parameters() if p.requires_grad)}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(swinir):
                ...
                controlnet_image = controlnet_image.clamp(0, 1)*2-1
                if args.pretrained_vae_model_name_or_path is not None:
                ...
                hr_latents = hr_latents * vae.config.scaling_factor
                ...
                bsz = hr_latents.shape[0]
                timesteps = torch.full((bsz,), 199, device=lr_latents.device)
                timesteps = timesteps.long()
                hr_latents = noise_scheduler.add_noise(hr_latents, noise, timesteps)
                lr_latents = noise_scheduler.add_noise(lr_latents, noise, timesteps)

                hr_latents = (1 / vae.config.scaling_factor) * hr_latents
                pixel_values  = vae.decode(hr_latents).sample
                pixel_values = (pixel_values / 2 + 0.5).clamp(0, 1)

                lr_latents = (1 / vae.config.scaling_factor) * lr_latents
                controlnet_image = vae.decode(lr_latents).sample
                controlnet_image = (controlnet_image / 2 + 0.5).clamp(0, 1)


                sr_image = swinir(controlnet_image)
                #可以尝试多种Loss

                loss = F.l1_loss(sr_image, pixel_values , reduction="sum")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        save_models(
                            accelerator.unwrap_model(swinir),
                            save_path,
                        )
